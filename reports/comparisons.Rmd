...
The maps above indicate that RegCM and REMO contribute in different ways to the full ensemble. Their different contribution is most evident in Figure 3. Overall, RegCM tends to run "cooler" (more days above threshold) over northern Africa than REMO. 
...
l_s[[2]] - select(l_s[[1]], c(2,5,8,11)) -> s_diff
...
The bootstrapped ensembles seem to obtain similar mean values to the full ensemble (REMO + RegCM), although they deviate more over the tails (5th and 95th percentiles). Both bootstrapped ensembles run "hotter" in the 5th percentile than the full ensemble (particularly over northern Africa), and "cooler" in the 95th percentile (particularly over eastern Europe). Between both bootstrapped ensembles mean values are quite similar, but the standard one is "cooler" than the parametric in the 5th percentile, and "warmer" in the 95th percentile.


Another note: the 1000 means (or any of the percentiles) I obtain from bootstrapping are theoretically normally distributed. For example, this is the distribution of the 63 "raw" observations of one cell:
